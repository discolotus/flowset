...
describe("semantic experiment runs", () => {
  ...
  it("creates an isolated immutable metadata-and-score snapshot", () => {
    const response: SemanticRankResponse = { backend, score_key: "semantic:focus", score_keys_by_normalized_label: { focus: "semantic:focus" }, results: [{ track_id: "a", status: "complete", scores: [{ key: "semantic:focus", label: "focus", normalized_label: "focus", score: 0.8, provenance: { backend: "local-clap", model: "clap-v1" } }] }], missing_track_ids: [] };
    const run = createTextRankingRun({ id: "run-1", labels: [" focus "], tracks: [track("a")], sourceTrackIds: ["a", "b"], backend, response, createdAt: "2026-08-21T10:00:00.000Z", completedAt: "2026-08-21T10:00:01.250Z" });
  ...
  it("keeps only a bounded list of recent runs", () => {
    const base: Omit<SemanticExperimentRunV1, "id"> = { schemaVersion: 1, createdAt: "", completedAt: "", durationMs: 0, kind: "text-ranking", status: "complete", backend, prompts: ["q"], scoreKeysByNormalizedLabel: { q: "key" }, query: "q", scoreKey: "key", trackIds: [], trackSetFingerprint: "fp", sourceTrackSetFingerprint: "source-fp", trackSnapshots: [], results: [], missingTrackIds: [], warnings: [] };
  ...
  it("keeps missing selected tracks visible as unavailable results", () => {
    const response: SemanticRankResponse = {
      ...
      score_keys_by_normalized_label: { focus: "semantic:focus" },
      results: [],
  ...
  it("rejects scores whose key or provenance escapes the response mapping", () => {
    const response: SemanticRankResponse = {
      backend,
      score_key: "semantic:focus",
      score_keys_by_normalized_label: { focus: "semantic:focus" },
      results: [{
        track_id: "a",
        status: "complete",
        scores: [{ key: "semantic:other", label: "focus", normalized_label: "focus", score: 0.8, provenance: { backend: "local-clap", model: "clap-v1" } }],
      }],
      missing_track_ids: [],
    };
    expect(() => createTextRankingRun({ id: "bad", labels: ["focus"], tracks: [track("a")], sourceTrackIds: ["a"], backend, response, createdAt: "2026-08-21T10:00:00.000Z", completedAt: "2026-08-21T10:00:01.000Z" }))
      .toThrow(/outside the requested prompt and model contract/);
  });
});
